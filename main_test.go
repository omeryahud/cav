...
func TestParseArgs(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "new bare is an error (name required)", args: []string{"-n"}, wantErr: true},
		{name: "new named", args: []string{"-n", "api", "fix"}, dir: cwd, sname: "api fix"},
		{name: "new attach without a name is an error", args: []string{"-n", "-a"}, wantErr: true},
		{name: "new named attach", args: []string{"-n", "x", "-a"}, dir: cwd, sname: "x", attach: true},
